@main def exec(cpgFile: String, outFile: String) = {
  ...
  def sinks = cpg.call.methodFullNameExact(
    "java.nio.file.Files.readAllBytes:byte[](java.nio.file.Path)"
  ).argument(1)
  val flows = sinks.reachableByFlows(sources).l
  val result = ujson.Obj(
    "schema_version" -> "2",
    "engine_id" -> "joern",
    "rule_id" -> "traceproof/joern-spring-get-file-path-v1",
    ...
    "qualification" -> "bounded_spring_file_path_discovery"
  )
