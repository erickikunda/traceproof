@main def exec(cpgFile: String, outFile: String) = {
  ...
  ).argument(1).isCall.nameExact("<operator>.arrayInitializer").filter { array =>
    array.argument.size == 3 &&
    Set("\"sh\"", "\"/bin/sh\"", "\"bash\"", "\"/bin/bash\"").contains(array.argument(1).code) &&
    array.argument(2).code == "\"-c\""
  }.argument(3)
  val flows = sinks.reachableByFlows(sources).l
  val result = ujson.Obj(
    "schema_version" -> "2",
    "engine_id" -> "joern",
    "rule_id" -> "traceproof/joern-spring-get-runtime-shell-v1",
    ...
    "qualification" -> "bounded_spring_runtime_shell_discovery"
  )
