@main def exec(cpgFile: String, outFile: String) = {
  importCpg(cpgFile)
  val argv = cpg.method.nameExact("main").filter(_.fullName.startsWith("main:")).parameter.index(2)
  ...
  def sinks = cpg.call.methodFullNameExact("<unresolvedNamespace>.system:<unresolvedSignature>(1)").filter(_ => !localSystem).argument(1)
  val flows = sinks.reachableByFlows(sources).l
  val result = ujson.Obj(
    "schema_version" -> "2", "engine_id" -> "joern",
    "rule_id" -> "traceproof/joern-cpp-argv-system-v1",
    "represented_cpp_files" -> ujson.Arr.from(cpg.file.name.l.filter(n => n.endsWith(".cpp") || n.endsWith(".h") || n.endsWith(".hpp")).distinct.sorted),
