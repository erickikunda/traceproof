"""Express endpoint claim binding; generated intermediate code is never an anchor."""

from traceproof.express_parser import parse_isolated
...
def anchors(bundle, snippet, claim):
    ...
    language = (
        "typescript"
        if bundle["rule_id"] == "traceproof/joern-typescript-express-eval-v1"
        else "javascript"
    )
    context = complete_context(bundle, snippet, ".ts" if language == "typescript" else ".js")
    if context is None:
        return []
    parsed = parse_isolated(context.encode(), language)
