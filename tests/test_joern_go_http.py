...
def test_go_http_profile_publication_and_retry(
    store, captured_source, tmp_path, monkeypatch, language, extension
):
    filename = f"app.{extension}"
    run = captured_source({filename: "package main\n"})
    home = tmp_path / "tools"
    ...
    assert report["discovery_profile"] == "go-http-shell-v1"
...
def test_go_http_rejects_other_languages(store, language):
    with pytest.raises(TraceProofError, match="profile/language"):
        joern.discover(
            ...
            "/unused",
            language=language,
            discovery_profile="go-http-shell-v1",
        )
