...
def test_express_profile_publication_and_retry(
    store, captured_source, tmp_path, monkeypatch, language, extension
):
    filename = f"app.{extension}"
    run = captured_source({filename: "eval(value);\n"})
    home = tmp_path / "tools"
                ...
                json.dumps(
                    {
                        "schema_version": "2",
                        "engine_id": "joern",
                        "rule_id": f"traceproof/joern-{language}-express-eval-v1",
    ...
    assert report["discovery_profile"] == "express-request-eval-v1"
    assert pipeline.scan_run(store, run, **options)["status"] == "skipped_existing_attempt"


@pytest.mark.parametrize("language", ["python", "java", "go", "c"])
def test_express_rejects_other_languages(store, language):
    with pytest.raises(TraceProofError, match="profile/language"):
        joern.discover(
            ...
            "/unused",
            language=language,
            discovery_profile="express-request-eval-v1",
        )
