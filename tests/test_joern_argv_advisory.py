...
def argv_bundle(request, store, captured_source, tmp_path, monkeypatch):
    language = request.param
    extension = language
    name = f"main.{extension}"
    source = (
        Path(__file__).parent / "fixtures/joern-argv" / language / "vulnerable" / name
    ).read_text()
                ...
                json.dumps(
                    {
                        ...
                        "paths": [
                            [
                                {"file": name, "line": 4, "code": "arguments[1]"},
                                {"file": name, "line": 4, "code": "command"},
                                {"file": name, "line": 5, "code": "command"},
                            ]
                        ],
                    }
                )
    ...
    result = joern.discover(
        store, run, home, language=language, discovery_profile="c-family-argv-system-v1"
    )
...
def test_explicit_policy_and_source_quotes(argv_bundle):
    b = argv_bundle
...
def test_evidence_and_decision_fail_closed(argv_bundle, change):
    b = deepcopy(argv_bundle)
    ...
    if change == "missing":
        ...
        for s in b["snippets"]:
            s["text"] = (
                s["text"].replace("<stdlib.h>", "<stdio.h>")
                if change == "foreign"
                else s["text"].replace("system(command)", "system(command, 0)")
            )
...
def test_cost_and_idempotency(store, argv_bundle, monkeypatch, stop):
    b = argv_bundle
    ...
    assert (
        triage(store, b["bundle_id"], policy(), adapter, "argv", review_policy=selected(b))
        == result
    )
    assert adapter.calls == (0 if stop else 1)


def test_main_workflow_and_report(store, argv_bundle, tmp_path):
    b = argv_bundle
    advisory = AdvisoryOptions(policy(), adapter_for(b), "argv-workflow", selected(b))
