...
from traceproof.joern_claims import GO_POLICY, assess_review_evidence, review_preflight
...
@pytest.fixture
def go_bundle(store, captured_source, tmp_path, monkeypatch):
    fixtures = Path(__file__).parent / "fixtures"
    run = captured_source({"main.go": (fixtures / "joern-go-http/vulnerable/main.go").read_text()})
    home = tmp_path / "tools"
    ...
    def fake(command, root, name, timeout):
        if name == "analyze":
            (root / "flows.json").write_bytes(
                (fixtures / "joern-go-advisory/native-flow.json").read_bytes()
            )

    monkeypatch.setattr(joern, "stage", fake)
    result = joern.discover(store, run, home, language="go", discovery_profile="go-http-shell-v1")
...
def test_literal_native_path_and_explicit_gate_only(go_bundle):
    b = go_bundle
    assert b["joern_native_audit"]["status"] == "validated"
    gate = assess_review_evidence(b, review_decision(b), GO_POLICY)
...
def test_bad_evidence_and_claims_fail_closed(go_bundle, change):
    b = deepcopy(go_bundle)
    ...
    if change == "missing":
        ...
        old, new = (
            ('"net/http"', '"bad/http"') if change == "source" else ('"os/exec"', '"bad/exec"')
        )
        for s in b["snippets"]:
    ...
    assert not assess_review_evidence(b, decision, GO_POLICY)["passed"]


def test_sink_argument_alone_does_not_satisfy_call_quote(go_bundle):
    decision = review_decision(go_bundle)
    decision = decision.model_copy(
        update={
            "claims": [
                decision.claims[0],
                decision.claims[1].model_copy(update={"quote": "value"}),
                *decision.claims[2:],
            ]
        }
    )
    assert not assess_review_evidence(go_bundle, decision, GO_POLICY)["passed"]


@pytest.mark.parametrize("stop", [None, "budget", "evidence"])
def test_opt_in_cost_gates_and_idempotency(store, go_bundle, monkeypatch, stop):
    b = go_bundle
    ...
    result = triage(store, b["bundle_id"], policy(), adapter, "go", review_policy=GO_POLICY)
    assert (
        result["state"]
    ...
    assert triage(store, b["bundle_id"], policy(), adapter, "go", review_policy=GO_POLICY) == result
    assert adapter.calls <= 1


def test_main_workflow_and_policy_scope(store, go_bundle, tmp_path):
    b = go_bundle
    advisory = AdvisoryOptions(policy(), adapter_for(b), "workflow", GO_POLICY)
    with pytest.raises(TraceProofError, match="profile"):
        advisory.validate_scope("go", None)
    with pytest.raises(TraceProofError, match="profile"):
        advisory.validate_scope("java", "go-http-shell-v1")
    ...
    result = pipeline.scan_run(
        ...
        joern_profile="go-http-shell-v1",
        advisory=advisory,
    )
...
def test_policy_in_provider_request_and_cli(store, go_bundle, tmp_path):
    from typer.testing import CliRunner

    from traceproof.cli import app

    b = go_bundle
    body = request_body(b, policy(), review_policy=GO_POLICY)
    assert GO_POLICY in json.dumps(body)
