# Separate networked acquisition container (through Slice 118)
...
## Private transport image refresh — Slice 118

The current image inventory now includes --ca-bundle, --proxy and --credential-file.
Mount CA and credential files read-only at operator paths such as /config and /secrets,
separate from source/handoff volumes; pass those paths as explicit image arguments.
The offline scanner must not receive these mounts. See [private HTTPS authentication](git-private-authentication.md).

Packaged validation passed single and CSV shallow Git smart-HTTP acquisition against a
synthetic loopback HTTPS server, invalid credentials/CA/repository rejection, and an HTTP
CONNECT proxy. The public acquisition/offline scan rehearsal also passed. No bank server,
HTTPS proxy, authenticated proxy or OCP admission is qualified by these tests.

To repeat the synthetic test from the checkout:

```sh
docker run --rm --network none --read-only --user 1000710000:0 \
  --cap-drop ALL --security-opt no-new-privileges --cpus 2 --memory 2g --pids-limit 256 \
  --tmpfs /work:rw,exec,nosuid,nodev,size=1g,mode=1777 \
  --tmpfs /tmp:rw,exec,nosuid,nodev,size=256m,mode=1777 \
  --mount type=bind,source="$PWD/scripts/validate_git_private_container.py",target=/validation.py,readonly \
  --entrypoint python traceproof:git-acquisition-linux-poc /validation.py
```

The server and proxy exist only inside disposable test scratch/loopback. This uses the
Docker runtime's ability to bind port 443 as non-root; no additional capabilities are granted.
