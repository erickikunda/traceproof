"""Native Rust environment-to-shell discovery and replay advisory; no live provider."""
...
from traceproof.joern_claims import RUST_POLICY, assess_review_evidence
...
try:
    for name, count in {
        ...
        for candidate in fetched["candidates"]:
            ...
            gate = assess_review_evidence(bundle, review_decision(bundle), RUST_POLICY)
            ...
            config = ModelConfig(
                provider="replay",
                model="synthetic-rust-workflow",
                input_micro_usd_per_1k=1000,
                output_micro_usd_per_1k=2000,
            )
            advisory = AdvisoryOptions(config, ReplayAdapter(reply), "rust-review", RUST_POLICY)
            ...
            assert history["review_policy"] == RUST_POLICY
