"""Opt-in source-only Go fixture probe; no production Go qualification."""
...
def main():
    ...
    for name in ("vulnerable", "fixed", "crossfile-vulnerable", "crossfile-fixed", "disconnected"):
        ...
        case.mkdir()
        source = case / "source"
        shutil.copytree(ROOT / "tests/fixtures/joern-go" / name, source)
        original = {
        ...
        prepare = run_stage(
            [str(home / "gosrc2cpg"), "-J-Xmx2g", str(source), "--output", str(case / "cpg.bin")],
            case,
            env,
            case / "prepare.log",
        )
        analyze = run_stage(
            [
                ...
                "-J-Xmx2g",
                "--script",
                str(ROOT / "scripts/joern/go-flow.sc"),
                "--param",
        ...
        results.append(
            dict(
                ...
                rule_sha256=hashlib.sha256(
                    (ROOT / "scripts/joern/go-flow.sc").read_bytes()
                ).hexdigest(),
                llm_calls=0,
                result=result,
