@main def exec(cpgFile: String, outFile: String) = {
  ...
  val result = ujson.Obj(
    ...
    "bindings" -> ujson.Arr.from(cpg.call.nameExact("Find").map(c => ujson.Obj(
      "code" -> c.code, "method_full_name" -> c.methodFullName,
      "callees" -> ujson.Arr.from(c.callee.filterNot(_.isExternal).fullName.l)
    )).l),
