"""Validate bounded Main Joern Spring advisory workflow discovery through the main pipeline."""
...
from traceproof.models import Decision, ModelConfig, ReplayAdapter
...
try:
    for name, count in {
        ...
        set_budget(store, run, 100000, max_requests=3)
        config = ModelConfig(
            provider="replay",
            model="synthetic-spring-workflow",
            input_micro_usd_per_1k=1000,
            output_micro_usd_per_1k=2000,
        )
        reply_name = name if name == "renamed" else "vulnerable"
        replay_path = fixtures.parent / "joern-spring-advisory" / f"{reply_name}.json"
        advisory = AdvisoryOptions(
            config, ReplayAdapter(replay_path), "workflow-advice", "joern-spring-review-v1"
        )
        options = dict(
            engine="joern",
            language="auto",
        ...
        assert attempt["language"] == "java" and attempt["model_calls"] == count
        ...
        published = get_report(store, name, report_id=attempt["report_id"])
        assert published["report_id"] == attempt["report_id"]
        assert attempt["live_model_calls"] == 0
        ledger = triage_report(store, run)
        assert ledger["total"] == count
        assert ledger["accounted_micro_usd"] == count * 140
        (root / f"{name}-advisory.json").write_text(json.dumps(ledger, indent=2))
        if count:
            history = published["candidates"][0]["triage_history"]
            assert history[-1]["review_policy"] == "joern-spring-review-v1"
            assert history[-1]["simulated"] and history[-1]["disposition"] == "needs_review"
        assert batch_result["model_calls"] == count and batch_result["live_model_calls"] == 0
