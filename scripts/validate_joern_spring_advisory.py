"""Validate bounded Joern Spring replay advisory discovery through the main pipeline."""
...
try:
    for name, count in {
        ...
        for candidate in fetched["candidates"]:
            ...
            config = ModelConfig(
                provider="replay",
                model="synthetic-spring-review",
                input_micro_usd_per_1k=1000,
                output_micro_usd_per_1k=2000,
            )
            replay_path = root / f"{name}-reply.json"
            ...
            adapter = ReplayAdapter(replay_path)
            result = triage(
                store,
                bundle["bundle_id"],
                config,
                adapter,
                "advisory",
                review_policy="joern-spring-review-v1",
            )
            ...
            published = publish_report(store, name, run, attempt["attempt_id"])
            assert (
                published["candidates"][0]["triage_history"][-1]["review_policy"]
                == "joern-spring-review-v1"
            )
            assert published["static_review_readiness"]["state"] == "incomplete"
            (root / f"{name}.html").write_text(render_report(published, "html"))
