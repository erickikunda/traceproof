"""Opt-in source-only C/C++ fixture probe; no production language qualification."""
...
def main():
    ...
    for name in ("vulnerable", "fixed", "crossfile-vulnerable", "crossfile-fixed", "disconnected"):
        ...
        case.mkdir()
        source = case / "source"
        shutil.copytree(ROOT / "tests/fixtures/joern-c-family" / args.language / name, source)
        original = {
        ...
        analyze = run_stage(
            [
                ...
                "-J-Xmx2g",
                "--script",
                str(ROOT / "scripts/joern/c-family-flow.sc"),
                "--param",
        ...
        results.append(
            dict(
                ...
                rule_sha256=hashlib.sha256(
                    (ROOT / "scripts/joern/c-family-flow.sc").read_bytes()
                ).hexdigest(),
                llm_calls=0,
                result=result,
