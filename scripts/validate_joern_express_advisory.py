"""Native Express discovery and explicit replay advisory; never invokes a live provider."""
...
POLICY = f"joern-{args.language}-express-review-v1"
root = args.output.resolve()
root.mkdir(parents=True, exist_ok=False)
fixtures = Path(__file__).resolve().parents[1] / "tests/fixtures/joern-express" / args.language
    ...
    for name, count in {
        ...
        with zipfile.ZipFile(archive, "w") as out:
            for file in (fixtures / name).rglob(
                "*.js" if args.language == "javascript" else "*.ts"
            ):
        ...
        assert attempt["language"] == args.language and attempt["model_calls"] == 0
        ...
        for candidate in fetched["candidates"]:
            ...
            gate = assess_review_evidence(bundle, review_decision(bundle), POLICY)
            ...
            config = ModelConfig(
                provider="replay",
                model="synthetic-express-workflow",
                input_micro_usd_per_1k=1000,
                output_micro_usd_per_1k=2000,
            )
            advisory = AdvisoryOptions(config, ReplayAdapter(reply), "express-review", POLICY)
            ...
            assert history["review_policy"] == POLICY
