"""Native Go HTTP-to-shell discovery and explicit replay advisory; never invokes a live provider."""
...
from traceproof.joern_claims import GO_POLICY, assess_review_evidence
...
fixtures = Path(__file__).resolve().parents[1] / "tests/fixtures/joern-go-http"
...
try:
    for name, count in {
        ...
        assert attempt["language"] == "go" and attempt["model_calls"] == 0
        ...
        for candidate in fetched["candidates"]:
            ...
            gate = assess_review_evidence(bundle, review_decision(bundle), GO_POLICY)
            ...
            config = ModelConfig(
                provider="replay",
                model="synthetic-go-workflow",
                input_micro_usd_per_1k=1000,
                output_micro_usd_per_1k=2000,
            )
            advisory = AdvisoryOptions(config, ReplayAdapter(reply), "go-review", GO_POLICY)
            ...
            assert history["review_policy"] == GO_POLICY
