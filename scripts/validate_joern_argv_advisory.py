"""Native C-family discovery and explicit replay advisory; never invokes a live provider."""
...
POLICY = f"joern-{args.language}-argv-review-v1"
root = args.output.resolve()
root.mkdir(parents=True, exist_ok=False)
fixtures = Path(__file__).resolve().parents[1] / "tests/fixtures/joern-argv" / args.language
...
try:
    for name, count in {
        ...
        for candidate in fetched["candidates"]:
            ...
            config = ModelConfig(
                provider="replay",
                model="synthetic-argv-workflow",
                input_micro_usd_per_1k=1000,
                output_micro_usd_per_1k=2000,
            )
            advisory = AdvisoryOptions(config, ReplayAdapter(reply), "argv-review", POLICY)
